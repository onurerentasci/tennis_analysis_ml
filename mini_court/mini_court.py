...
class MiniCourt:
    ...
    def __init__(self, frame):
        self.drawing_rectangle_width = 250  # çizim alanının genişliği
        self.drawing_rectangle_height = 500  # çizim alanının yüksekliği
        self.buffer = 50  # çizim alanı ile video karesi arasındaki boşluk
        self.padding_court = (
            20  # mini kortun kenarları ile çizim alanı arasındaki boşluk
        )
    ...
    def set_canvas_background_box_position(self, frame):
        frame = frame.copy()
        # çizim alanının pozisyonlarını belirleme işlemi, çizim alanı video karesinin sağ üst köşesine yerleştirilir

        self.end_x = frame.shape[1] - self.buffer
        # frame.shape[1] -> karenin genişliği
        self.end_y = self.buffer + self.drawing_rectangle_height
        # frame.shape[0] -> karenin yüksekliği
        self.start_x = self.end_x - self.drawing_rectangle_width
        # çizim alanının genişliği
        self.start_y = self.end_y - self.drawing_rectangle_height
        # çizim alanının yüksekliği
    ...
    def draw_court(self, frame):
        for i in range(0, len(self.drawing_key_points), 2):
            ...
            cv2.circle(
                img=frame, center=(x, y), radius=5, color=(0, 0, 255), thickness=-1
            )  # cv2.circle() -> çember çizme işlemi

        # çizgileri çizme işlemi
        for line in self.lines:
            # çizgilerin çizilmesi, çizgilerin başlangıç ve bitiş noktaları alınır
            start_point = (
            ...
            # start_point -> çizginin başlangıç noktası
            # end_point -> çizginin bitiş noktası

            cv2.line(
                img=frame, pt1=start_point, pt2=end_point, color=(0, 0, 0), thickness=2
            )

        # ağ çizgisi çizme işlemi
        net_start_point = (
    ...
    def draw_background_rectangle(self, frame):
        ...
        shapes = np.zeros_like(
            frame, np.uint8
        )  # frame ile aynı boyutta bir matris oluşturulur
        # neden? çizim alanı oluşturulur, bu çizim alanı frame ile aynı boyutta olmalıdır
        ...
        cv2.rectangle(
            ...
            (255, 255, 255),
            cv2.FILLED,  # cv2.FILLED -> içi dolu çizim yapar
        )

        out = frame.copy()
        # frame kopyalanır, çünkü çizim işlemi frame üzerinde yapılacak
        alpha = 0.5
        # alpha -> çizimlerin saydamlığı
        mask = shapes.astype(bool)
        # shapes matrisi boolean türüne dönüştürülür, bu alanın dışındaki kısımlar saydam olmalıdır, bu yüzden mask kullanılır
        out[mask] = cv2.addWeighted(frame, alpha, shapes, 1 - alpha, 0)[mask]
        # cv2.addWeighted() -> iki resmi birleştirir
        # frame -> birinci resim
        # shapes -> ikinci resim
        # mask -> alanın dışındaki kısımlar saydam olmalıdır
        return out
    ...
    def get_mini_court_coordinates(
        ...
        distance_from_keypoint_x_pixels, distance_from_keypoint_y_pixels = (
            mesaure_xy_distance(object_position, closest_keypoint)
        )

        # pixel mesafereyi metre cinsine çevirme işlemi
        ...
        # mini kort koordinatlarını döndürme işlemi
